...
final readonly class DocCheckOAuthMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if (!$frontendUser instanceof FrontendUserAuthentication) {
            return new HtmlResponse('DocCheck Login is unavailable.', 503);
        }
        ...
        try {
            ...
            return new HtmlResponse('DocCheck Login could not be completed. Please try again later.', 400);
        }
    }
    ...
    private function handleCallback(ServerRequestInterface $request, \DocCheck\OAuth2DocCheckTypo3\Configuration\DocCheckConfiguration $configuration, FrontendUserAuthentication $frontendUser): ResponseInterface
    {
        ...
        if ($configuration->licenseMode() !== 'basic') {
            ...
            if (!is_string($parameters['state'] ?? null) || $parameters['state'] === '') {
                ...
                return new HtmlResponse('DocCheck Login callback did not include the required security state. Start a new login from the login page.', 400);
            }
            $transaction = $transactionStore->consume($parameters['state']);
            if ($transaction === null) {
                return new HtmlResponse('The DocCheck login transaction expired or was replaced. Start a new login from the login page.', 400);
            }
            if (isset($parameters['error']) || !is_string($parameters['code'] ?? null) || $parameters['code'] === '' || !$transaction->matches($parameters['state'], new \DateTimeImmutable())) {
                return new HtmlResponse('DocCheck Login was cancelled or could not be completed.', 400);
            }
        ...
        try {
            ...
            return new HtmlResponse('DocCheck Login could not verify the authorization response. Check the active client and exact callback URI, then try again.', 400);
        }
        if ($configuration->licenseMode() !== 'basic') {
            if (!$configuration->isFrontendUserProvisioningEnabled()) {
                if (!$configuration->isAnonymousSessionFallbackAllowed()) {
                    return new HtmlResponse('DocCheck Login requires enabled frontend-user provisioning.', 503);
                }
            ...
            try {
                ...
                return new HtmlResponse('DocCheck Login could not retrieve the consented profile data. Check the selected licence scopes and consent, then try again.', 400);
            }
            ...
            try {
                ...
                return new HtmlResponse('DocCheck Login verified your profile but could not establish the local DocCheck session. Please try again.', 500);
            }
